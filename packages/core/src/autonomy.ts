...
export function buildAutonomousPromotedGoalNotes(
  itemId: string,
  notes: string,
  now: Date = new Date(),
): string {
  ...
  return appendLineageNotes(notes, [
    ...
    `sourceIdeaId=${itemId}`,
    `depth=${metadata?.depth ?? 1}`,
    `autonomousAt=${now.toISOString()}`,
  ]);
}

export function isAutonomousGoalNotes(notes: string): boolean {
  return notes.includes(AUTONOMOUS_GOAL_MARKER);
}

export function isUserPromotedGoalNotes(notes: string): boolean {
  return notes.includes(USER_PROMOTED_GOAL_MARKER);
}
...
/** Prefer explicit autonomousAt= lineage; fall back to row createdAt. */
export function autonomousGoalCreatedAt(notes: string, createdAt: Date): Date {
  const raw = metadataValue(notes, "autonomousAt");
  if (raw) {
...
export function wasAutonomousGoalCreatedOnUtcDay(
  notes: string,
  createdAt: Date,
  now: Date = new Date(),
): boolean {
  if (!isAutonomousGoalNotes(notes)) return false;
  const start = utcDayStart(now);
  const end = new Date(start.getTime() + 86_400_000);
  const at = autonomousGoalCreatedAt(notes, createdAt);
  return at >= start && at < end;
}

export function countAutonomousGoalsForUtcDay(
...
/**
 * True when this write newly introduces an autonomous open goal:
 * open non-idea with the autonomous marker, or idea→goal promote without the user-promoted marker.
 */
export function isIntroducingAutonomousGoal(input: AutonomousGoalIntroduction): boolean {
  if (input.status !== "open") return false;
  if (isIdeaTitle(input.title)) return false;
  if (isUserPromotedGoalNotes(input.notes)) return false;

  const alreadyAutonomous =
    input.previousStatus === "open" &&
    input.previousTitle !== undefined &&
    !isIdeaTitle(input.previousTitle) &&
    isAutonomousGoalNotes(input.previousNotes ?? "");
  if (alreadyAutonomous) return false;

  if (isAutonomousGoalNotes(input.notes)) return true;
  if (input.previousTitle !== undefined && isIdeaTitle(input.previousTitle)) return true;
  return false;
}

/** Ensure autonomous marker + autonomousAt lineage on notes (idempotent when already stamped). */
export function ensureAutonomousPromotedGoalNotes(
  itemId: string,
  notes: string,
  now: Date = new Date(),
): string {
...
export function autonomyLimitError(args: {
  ...
  autonomousGoalsToday: number;
}): string | null {
  if (exceedsGoalChainDepth(args.notes, args.limits.maxChainDepth)) {
    return `Autonomy goal chain depth limit is ${args.limits.maxChainDepth}.`;
  }
  if (args.autonomousGoalsToday >= args.limits.maxGoalsPerDay) {
    return `Autonomy daily goal limit is ${args.limits.maxGoalsPerDay}.`;
  }
  return null;
}
