...
export function workspaceFileAccessPython(): string {
  return [
    ...
    "    if getpath is None: raise",
    "    buf = bytearray(1024)  # Darwin MAXPATHLEN",
    "    fcntl.fcntl(fd, getpath, buf)",
    "    return os.path.realpath(os.fsdecode(bytes(buf)).rstrip('\\0'))",
